...
The git version tag 1.0 corresponds to the version used in the published paper.
...
3. Run `py/find_relevant_releases_finetune.py` - creates `results/press_releases/relevant_press_releases.csv` by first finetuning a DistilBERT model on a small labeled dataset of relevant and non-relevant press releases, and then using the model to score the press releases in `data/filtered_press_releases.csv` (appends if it exists)
4. Run `py/extract_product_capabilities.py` - creates `results/press_releases/processed_press_releases.csv` (appends if it exists) and `results/press_releases/ai_product_press_releases.csv`. Executes GPT queries.
5. Run `py/match_capabilities_to_skills.py` - creates `results/scored_esco_skills.csv` (overwrites)
6. Run `py/scrape_cedefop.py` - creates `data/skills_intelligence_data.csv` (overwrites)
7. Run `R/aggregate_skills_to_occupations.R` - creates `results/scored_esco_occuations.csv`
8. Run `R/match_to_prior_work.R` - creates `results/scored_esco_occupations_matched.csv`
